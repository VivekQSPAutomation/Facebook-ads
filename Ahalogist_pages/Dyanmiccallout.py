...
class Dycall(Basepages):
    ...
    link_url =(By.XPATH,"//label[contains(text(),'In-line DRC Text')]/following-sibling::input")
    Download_image = (By.XPATH,"//button[contains(text(),'Download Image')]")
    Download_inline = (By.XPATH,"//button[contains(text(),'Download In-Line Snippet')]")
    ...
    def dynamic_callout(self):
        ...
        self.do_send_keys(self.DRC_name,f"Test-check_{''.join(secrets.choice(string.ascii_letters + string.digits) for _ in range(10))}_{formatted_datetime}")
