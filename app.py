...
df.dropna(subset=['lat', 'lon'], inplace=True)  # Drop rows with missing values
...
def filter_postcodes(lat, long, distance_threshold):
    ...
    dist = 3959 * c  # Radius of Earth in miles
    postcodes = df.loc[dist <= distance_threshold, 'code'].tolist()
    return postcodes
...
async def upload_csv(request: Request, file: UploadFile = File(...), radius: int = Form(...)):
    ...
    with open(csv_file, 'w', newline='') as csvfile:
        ...
        for lat, long, code in json_string:
            ...
            print(extended_zip_codes)
            writer.writerow({'ZipCode': code, 'Extended Zipcodes': extended_zip_codes})
